...
export default function ExperiencesComp() {
  ...
  const [optimisticExperiences] = useOptimisticList(data || []);
  ...
  return (
    ...
    >
      ...
      <div className="relative w-full max-w-4xl">
        {/* Vertical Line */}
        <div className="absolute left-1/2 w-1 bg-slate-200 h-full transform -translate-x-1/2"></div>

        {/* Timeline Items */}
        <ul className="space-y-12">
          {optimisticExperiences.map((item, index) => {
            const startYear = new Date(item.start_date || "").getFullYear();
            const endYear = item.currently_working
              ? "Currently Working"
              : item.end_date
            ...
            return (
              ...
              >
                {/* Left side (year, company, position) */}
                <div
                  className={`w-1/2 ${
